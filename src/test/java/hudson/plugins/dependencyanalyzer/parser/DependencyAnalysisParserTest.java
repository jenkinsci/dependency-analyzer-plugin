...
public class DependencyAnalysisParserTest extends AbstractParserTestUtils {
	...
	public void testParseDependencyAnalyzeSection() throws Exception {
		File file = this.getFile("dependency_analyze_section_1");
		FileReader reader = new FileReader(file);

		String content = IOUtils.toString(reader);

		Map<DependencyProblemType, List<String>> result = DependencyAnalysisParser.getInstance()
				.parseDependencyAnalyzeSection(content);

		Assert.assertEquals("Must have unsused declared and used undeclared dependencies", 2, result.size());

		List<String> list = result.get(DependencyProblemType.UNUSED);
		Assert.assertEquals("Must have 2 unused declared dependencies", 2, list.size());
